 ...
 */
class FacultyController extends AsosController
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $post=Faculty::find()->where(['id'=>$id])->one();
        if (Faculty::STATUS_SHOWED==$post->status){
            $post->status=Faculty::STATUS_NOTSHOWED;
        }
        else{
            $post->status=Faculty::STATUS_SHOWED;
        }
        $post->save();
        return $this->redirect(['index']);
    }
