 ...
 */
class DepartmentsController extends AsosController
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $post=Departments::find()->where(['id'=>$id])->one();
        if (Departments::STATUS_SHOWED==$post->status){
            $post->status=Departments::STATUS_NOTSHOWED;
        }
        else{
            $post->status=Departments::STATUS_SHOWED;
        }
        $post->save();
        return $this->redirect(['index']);
    }
